...
const isPostMine = async (session, post) => {
  ...
  return s && s.length > 0 && s[0].user == session ? true : false
}
...
const deletePost = async ({ post, when }) => {
  try {
    ...
    let postRows = await db.query(
      'SELECT imgSrc FROM posts WHERE post_id=?',
      [post]
    )
    if (postRows && postRows.length > 0) {
      let imgSrc = postRows[0].imgSrc
      if (imgSrc) {
        let postPath = `${root}/dist/posts/${imgSrc}`
        try {
          const fs = require('fs')
          if (fs.existsSync(postPath)) {
            await promisify(unlink)(postPath)
          }
        } catch (e) {}
      }
    }
