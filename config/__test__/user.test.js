...
describe('User config tests', () => {
  ...
  test('should return ID of steve_jobs', async () => {
    let id = await User.getId('steve_jobs')
    expect(id).toBeNumber()
    expect(id).toBe(user2)
  })

  test('should return firstname & surname of ID 30', async () => {
    ...
    expect(surname).toEqual('doraemon')
  })

  test('should safely deactivate and delete a user and their associated records without errors', async () => {
    let tempUser = await User.create_user({
      username: 'deactivate_test_user',
      firstname: 'Deact',
      surname: 'Test',
      email: 'deact_test@gmail.com',
      password: 'testPassword',
      joined: new Date().getTime(),
      email_verified: 'no',
      isOnline: 'no',
    })
    const req = { cookies: {}, session: { id: tempUser.insertId, reset: jest.fn() } }
    const res = { cookie: jest.fn() }

    await expect(User.deactivate(tempUser.insertId, req, res)).resolves.not.toThrow()

    let exists = await User.getId('deactivate_test_user')
