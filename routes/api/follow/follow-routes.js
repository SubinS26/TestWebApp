...
app.post('/search-followings', async (req, res) => {
  let id = req.session ? req.session.id : 0
  let followings = await db.query(
  ...
  let allUsers = await db.query(
    'SELECT id AS follow_to, username AS follow_to_username FROM users WHERE id <> ? ORDER BY username ASC',
    [id]
  )
  let combined = [...followings]
  for (let u of allUsers) {
    if (!combined.some(c => c.follow_to === u.follow_to)) {
      combined.push(u)
    }
  }
  res.json(combined)
